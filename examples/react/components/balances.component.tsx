...
const BalancesComponent = (props: IBalancesComponent) => {
    ...
    const handleStreamingClick = useCallback(async () => {
        try {
            ...
            setSubscription(balanceObserver.subscribe((data) => {
    ...
    return (
        <Box>
            ...
            {balanceError &&
                <Typography color="red">{balanceError}</Typography>
            }

            {lastUpdate &&
                <Typography color="gray">{lastUpdate.toDateString()} {lastUpdate.toTimeString()}</Typography>
            }
