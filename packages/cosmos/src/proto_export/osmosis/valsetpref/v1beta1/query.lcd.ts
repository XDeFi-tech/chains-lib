...
import {
  ...
  UserValidatorPreferencesResponseSDKType,
} from './query';
export class LCDQueryClient {
  ...
  async userValidatorPreferences(
    params: UserValidatorPreferencesRequest
  ): Promise<UserValidatorPreferencesResponseSDKType> {
    const endpoint = `osmosis/valset-pref/v1beta1/${params.address}`;
    return await this.req.get<UserValidatorPreferencesResponseSDKType>(
      endpoint
    );
  }
}
