...
export class QueryClientImpl implements Query {
  ...
  userValidatorPreferences(
    ...
    return promise.then((data) =>
      UserValidatorPreferencesResponse.decode(new BinaryReader(data))
    );
  }
}
...
export interface UseUserValidatorPreferencesQuery<TData>
  extends ReactQueryParams<UserValidatorPreferencesResponse, TData> {
  request: UserValidatorPreferencesRequest;
}
...
export const createRpcQueryHooks = (rpc: ProtobufRpcClient | undefined) => {
  ...
  const useUserValidatorPreferences = <
    TData = UserValidatorPreferencesResponse
  >({
    request,
    options,
  }: UseUserValidatorPreferencesQuery<TData>) => {
    return useQuery<UserValidatorPreferencesResponse, Error, TData>(
      ['userValidatorPreferencesQuery', request],
      () => {
