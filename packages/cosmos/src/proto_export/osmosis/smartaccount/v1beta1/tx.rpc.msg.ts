...
export interface Msg {
  ...
  ): Promise<MsgRemoveAuthenticatorResponse>;
  /**
  ...
  ): Promise<MsgSetActiveStateResponse>;
}
export class MsgClientImpl implements Msg {
  ...
  addAuthenticator(
    ...
    const promise = this.rpc.request(
      'osmosis.smartaccount.v1beta1.Msg',
      'AddAuthenticator',
      data
    );
    return promise.then((data) =>
      MsgAddAuthenticatorResponse.decode(new BinaryReader(data))
    );
  }
  removeAuthenticator(
    request: MsgRemoveAuthenticator
    ...
    return promise.then((data) =>
      MsgRemoveAuthenticatorResponse.decode(new BinaryReader(data))
    );
  }
  setActiveState(
    request: MsgSetActiveState
    ...
    return promise.then((data) =>
      MsgSetActiveStateResponse.decode(new BinaryReader(data))
    );
  }
}
