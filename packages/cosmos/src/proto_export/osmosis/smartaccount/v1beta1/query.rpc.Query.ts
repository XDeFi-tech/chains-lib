...
export interface UseGetAuthenticatorsQuery<TData>
  extends ReactQueryParams<GetAuthenticatorsResponse, TData> {
  request: GetAuthenticatorsRequest;
}
...
export const createRpcQueryHooks = (rpc: ProtobufRpcClient | undefined) => {
  ...
  const useGetAuthenticators = <TData = GetAuthenticatorsResponse>({
    ...
    return useQuery<GetAuthenticatorsResponse, Error, TData>(
      ['getAuthenticatorsQuery', request],
      () => {
        if (!queryService) throw new Error('Query Service not initialized');
        return queryService.getAuthenticators(request);
      },
