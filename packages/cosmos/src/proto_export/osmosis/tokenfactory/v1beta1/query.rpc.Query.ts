...
export class QueryClientImpl implements Query {
  ...
  denomsFromCreator(
    ...
    return promise.then((data) =>
      QueryDenomsFromCreatorResponse.decode(new BinaryReader(data))
    );
  }
  beforeSendHookAddress(
    request: QueryBeforeSendHookAddressRequest
    ...
    return promise.then((data) =>
      QueryBeforeSendHookAddressResponse.decode(new BinaryReader(data))
    );
  }
  allBeforeSendHooksAddresses(
    request: QueryAllBeforeSendHooksAddressesRequest = {}
    ...
    const data =
      QueryAllBeforeSendHooksAddressesRequest.encode(request).finish();
    const promise = this.rpc.request(
      'osmosis.tokenfactory.v1beta1.Query',
      'AllBeforeSendHooksAddresses',
      data
    ...
    return promise.then((data) =>
      QueryAllBeforeSendHooksAddressesResponse.decode(new BinaryReader(data))
    );
  }
}
...
export interface UseDenomAuthorityMetadataQuery<TData>
  extends ReactQueryParams<QueryDenomAuthorityMetadataResponse, TData> {
  request: QueryDenomAuthorityMetadataRequest;
}
export interface UseDenomsFromCreatorQuery<TData>
  extends ReactQueryParams<QueryDenomsFromCreatorResponse, TData> {
  request: QueryDenomsFromCreatorRequest;
}
export interface UseBeforeSendHookAddressQuery<TData>
  extends ReactQueryParams<QueryBeforeSendHookAddressResponse, TData> {
  request: QueryBeforeSendHookAddressRequest;
}
export interface UseAllBeforeSendHooksAddressesQuery<TData>
  extends ReactQueryParams<QueryAllBeforeSendHooksAddressesResponse, TData> {
  request?: QueryAllBeforeSendHooksAddressesRequest;
}
...
export const createRpcQueryHooks = (rpc: ProtobufRpcClient | undefined) => {
  ...
  const useDenomsFromCreator = <TData = QueryDenomsFromCreatorResponse>({
    ...
    return useQuery<QueryDenomsFromCreatorResponse, Error, TData>(
      ['denomsFromCreatorQuery', request],
      () => {
        if (!queryService) throw new Error('Query Service not initialized');
        return queryService.denomsFromCreator(request);
      },
  ...
  const useBeforeSendHookAddress = <
    TData = QueryBeforeSendHookAddressResponse
  >({
    request,
    options,
  }: UseBeforeSendHookAddressQuery<TData>) => {
    return useQuery<QueryBeforeSendHookAddressResponse, Error, TData>(
      ['beforeSendHookAddressQuery', request],
      () => {
        if (!queryService) throw new Error('Query Service not initialized');
        return queryService.beforeSendHookAddress(request);
      },
  ...
  const useAllBeforeSendHooksAddresses = <
    TData = QueryAllBeforeSendHooksAddressesResponse
  >({
    request,
    options,
  }: UseAllBeforeSendHooksAddressesQuery<TData>) => {
    return useQuery<QueryAllBeforeSendHooksAddressesResponse, Error, TData>(
      ['allBeforeSendHooksAddressesQuery', request],
      () => {
        if (!queryService) throw new Error('Query Service not initialized');
        return queryService.allBeforeSendHooksAddresses(request);
      },
  ...
  return {
     ...
     * hook addresse's idx.
     */
    useAllBeforeSendHooksAddresses,
  };
};
