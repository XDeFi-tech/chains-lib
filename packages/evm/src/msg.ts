import { GasFeeSpeed, HexString, Msg as BasMsg, NumberIsh, utils } from '@xdefi/chains-core';
...
import { ERC1155_SAFE_TRANSFER_METHOD, ERC721_SAFE_TRANSFER_METHOD } from './consts';
...
export class ChainMsg extends BasMsg<MsgBody, TxData, FeeEstimation> {
    getFee(): FeeEstimation {
        ...
        if (this.data.maxFeePerGas) {
            ...
            const maxFee = ethers.utils.parseUnits(String(this.data.maxFeePerGas), 'gwei');
            const priorityFee = ethers.utils.parseUnits(String(this.data.maxPriorityFeePerGas), 'gwei');
            const maxFeeWithPriority = maxFee.add(priorityFee);
            estimation.fee = ethers.utils.formatUnits(maxFee.mul(this.data.gasLimit), 'ether').toString()
            estimation.maxFee = ethers.utils.formatUnits(maxFeeWithPriority.mul(this.data.gasLimit), 'ether').toString();
        } else {
            if (!this.data.gasPrice || !this.data.gasLimit) {
                return estimation;
            }
            const gasPrice = ethers.utils.parseUnits(String(this.data.gasPrice), 'gwei');
    ...
    async buildTx(): Promise<TxData> {
        ...
        if (this.provider) {
            if (!msgData.gasLimit) {
                const [feeData] = await this.provider.estimateFee([this], GasFeeSpeed.medium);
        ...
        if (msgData.maxFeePerGas && msgData.maxPriorityFeePerGas) {
            baseTx.maxFeePerGas = utils.toHex(
                ethers.utils.parseUnits(msgData.maxFeePerGas.toString(), 'gwei').toString()
            );
            baseTx.maxPriorityFeePerGas = utils.toHex(
                ethers.utils.parseUnits(msgData.maxPriorityFeePerGas.toString(), 'gwei').toString()
            );
            baseTx.type = 2;
        } else {
        ...
        switch (this.data.tokenType) {
            ...
            case TokenType.ERC721:
                ...
                populatedTx = await contract.populateTransaction[ERC721_SAFE_TRANSFER_METHOD](
                    msgData.from,
                    msgData.to,
                    msgData.nftId,
                    {
            ...
            case TokenType.ERC1155:
                ...
                populatedTx = await contract.populateTransaction[ERC1155_SAFE_TRANSFER_METHOD](
                    msgData.from,
                    msgData.to,
                    msgData.nftId,
                    1,
                ...
                break;
            default:
                baseTx.value = ethers.utils.parseEther(msgData.amount.toString()).toHexString();
        }
